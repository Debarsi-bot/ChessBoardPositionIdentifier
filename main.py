...
from imageToFen import ImageToFenConverter


#* scripts/main/test/image1.png

# CHESS_ENGINE_PATH = "scripts/main/chessEngineBin/stockfish_15_x64_avx2.exe"
CHESS_ENGINE_PATH = "chessEngineBin/bluebetafish_64bit_windows.exe"


emptyNonemptySquareClassifier = load_model("trainedModels/emptyNonemptySquareClassifier/CNN/trainedModelCNN.h5" ) 
blackOrWhitePieceClassifier = load_model("trainedModels/blackOrWhitePieceClassifier/CNN/trainedModelCNN.h5" ) 
pieceTypeClassifier = load_model("trainedModels/pieceTypeClassifier/NewTrainingData/trainedModelCNN.h5" )
...
def calculate(img, currentPlayer = "b", moveTime = 3000):
    # currentPlayer = input("Enter current player (w/b) : ")
    # moveTime = input("Enter movetime(in ms) : ")
    startTime = time.process_time()
    fenString = imageToFenConverter.getFenFromImage(img)  + " " + currentPlayer + " - - "
    endTime = time.process_time()
    ...
    print("\n\nPrinting analysis: ")
    print("-----------------------------------------------------------------------\n")
    bestMoveString, depthMessage = chessEngine.go(moveTime)
    return bestMoveString,fenString,depthMessage
